...
namespace gameswf
{
	...
	// For holding a pre-tesselated shape.
	{
		...
		///!!!Plarium
        void set_slice_index(int value) { m_slice_index = value; }
        int get_slice_index() const { return m_slice_index; }
        const array<coord_component>& get_coords() const { return m_triangle_strip; }
	...
	// certain error tolerance.
	{
		...
		void display( const matrix& m, const cxform& cx, const array<fill_style>& fills,
			const array<line_style>& line_styles, render_handler::bitmap_blend_mode bm, const scaling_grid* scalingGrid = NULL) const; ///!!!Plarium
    ...
    struct slice9_mesh_set : public mesh_set ///!!!Plarium
    {
             slice9_mesh_set(const tesselate::tesselating_shape* sh, float error_tolerance);
        void sort_slices(scaling_grid* scalingGrid);
    };
	...
	// information on fill and line styles.
	{
		...
		void	display( const matrix& mat, const cxform& cx, float pixel_scale, const array<fill_style>& fill_styles,
				const array<line_style>& line_styles, render_handler::bitmap_blend_mode bm, const character *scalingGrid = NULL) const; ///!!!Plarium
