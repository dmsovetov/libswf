...
namespace gameswf
{
	...
	//	array<bool>	m_init_actions_executed;	// a bit-array class would be ideal for this
        ...
        ///!!!Plarium
        array<bool>             m_init_tags_executed;
        array<action_buffer*>   m_init_actions;
        ///!!!Plarium
    #endif
        ...
        bool m_is_constructed;  ///!!!Plarium
		gc_ptr<character> m_canvas;

		// flash9
		hash<int, gc_ptr<Function> >* m_script;	// <frame, script>
		gc_ptr<Function> m_frame_script;	// current script
...
//		virtual bool	has_looped() const { return m_has_looped; }
		...
		virtual bool get_topmost_mouse_entity( character * &te, float x, float y, bool shouldHandleMouse = true);   ///!!!Plarium - shouldHandleMouse
		void advance(float delta_time);

		virtual void	alive();
    #if ENABLE_AS2
        void    execute_init_tags(int frame);                   ///!!!Plarium
        void    execute_postponed_actions(float delta_time);    ///!!!Plarium
    #endif
        void    do_init_actions();  ///!!!Plarium
        ...
        virtual const char* to_string() { static char str[64]; sprintf( str, "[MovieClip %s]", m_name.c_str() ); return str; }

		virtual bool	set_member(const tu_string& name, const Value& val);
		virtual bool	get_member(const tu_string& name, Value* val);
		virtual void	call_frame_actions(const Value& frame_spec);
		...
		void local_to_global(Object* pt);
		void global_to_local(Object* pt);
		...
		// flash9
		void add_script(int frame, Function* func);

        void invokeConstructor( void );
	};
}
