...
namespace gameswf
{
	...
	{
        assert(root_movie != NULL);

		m_movie = root_movie;
        m_movie->set_name("root");
	}
	...
	void	root::advance(float delta_time)
	{
        ...
        m_movie->get_topmost_mouse_entity(te, PIXELS_TO_TWIPS(m_mouse_x), PIXELS_TO_TWIPS(m_mouse_y), false);
        m_mouse_button_state.m_intersected_entity = te;
	...
	//	m_listener.advance(delta_time);
		...
		if (m_time_remainder >= m_frame_time)
		{
			...
			{
                sprite_instance* movie = cast_to<sprite_instance>(m_movie);

				set_flash_vars(m_player->m_flash_vars);

                movie->do_init_actions();    ///!!!Plarium

				if (m_def->m_is_avm2)
				{
                    movie->invokeConstructor();

                    movie->dispatchEvent( new Event( m_player.get(), "ADDED" ) );
                    movie->dispatchEvent( new Event( m_player.get(), "ADDED_TO_STAGE" ) );
				}
                else
                {
                    m_movie->construct();	///!!!Plarium
                }
			}
	...
	tu_string root::call_method(const char* method_name, Value * arguments, int argument_count )
	{
		assert(m_movie != NULL);
