...
//	struct Object;
    ...
    struct scaling_grid;	///!!!Plarium
    struct file_download;   ///!!!Plarium

    // ** AVM2
    #define AVM2_DEBUG  (1)
    #define AVM2_INTERNAL_BUILTIN (1)
    #define AVM2_VERBOSE_INSTRUCTIONS (1)
    #define AVM2_VERBOSE( ... ) IF_VERBOSE_ACTION( log_msg( __VA_ARGS__ ) )
    ...
    #define AvmBeginClass( type )                   class type##Closure {       \
                                                    public:                     \
                                                        AvmDeclareNew( type )
    #define AvmEndClass                             };
    #define AvmDeclareNew( type )                   static Object*  newOp( player* player ) { return new type( player ); }
    ...
    AvmDeclarePtrs( Package, Packages )
    AvmDeclarePtrs( Namespace, Namespaces )
    AvmDeclarePtrs( Multiname, Multinames )
    ...
    typedef string_hash<PackagePtr>                 PackageRegistry;
    ...
    typedef Object* (*CreateInstanceThunk)( player* player );
    // ** AVM2
    ...
    typedef file_download* (*file_download_callback)(const char* identifier);                               ///!!!Plarium
	exported_module void                 register_file_opener_callback(file_opener_callback opener);
    exported_module void                 register_file_download_callback(file_download_callback callback);  ///!!!Plarium
    exported_module file_opener_callback get_file_opener_callback();                                        ///!!!Plarium
    exported_module file_download_callback get_file_download_callback();                                    ///!!!Plarium
    ...
    exported_module render_handler*	create_render_handler_ogles2();
	...
	struct Object_interface : public ref_counted
	{
	...
	cast_class* cast_to(Object_interface* object)
	{
		if (object)
		{
	...
	template <typename cast_class>
	const cast_class* cast_to(const Object_interface* object)
	{
		if (object)
		{
...
//		create_font_shapes_flag cfs);
	...
	{
		// audio for video
		typedef void (*aux_streamer_ptr)(Object* netstream, unsigned char* stream, int len);
		...
		virtual void	play_sound(Object* listener_obj, int sound_handle, int loop_count /* , volume, pan, etc? */) = 0;
		...
		virtual void attach_aux_streamer(aux_streamer_ptr ptr, Object* netstream) {};
		virtual void detach_aux_streamer(Object* netstream) {};
		virtual void cvt(short int** adjusted_data, int* adjusted_size, unsigned char* data, int size, int channels, int freq) {};
	...
	struct bitmap_info : public ref_counted
	{
        ...
        virtual bool update(int w, int h, char *data) { return false; }     ///!!!Plarium
        virtual void set_identifier(const tu_string& value) {}              ///!!!Plarium
    ...
    struct file_download : public ref_counted    ///!!!Plarium
    {
                                    file_download( const tu_string& path ) : m_path( path ) {}
        virtual                     ~file_download() {}

        virtual tu_file*            get_file() { return NULL; }
        virtual const tu_string&    get_path() const { return m_path; }
        virtual int                 get_bytes_loaded() { return 0; }
        virtual int                 get_bytes_total() { return 0; }
	...
	struct render_handler
	{
		...
		///!!!Plarium
        virtual void    set_scaling_grid(const scaling_grid* scalingGrid) = 0;
