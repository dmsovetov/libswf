...
namespace gameswf
{
    extern bool s_tag_loaders_registered;
	...
	{
		s_tag_loaders.clear();
        s_tag_loaders_registered = false;
	}
	...
	void	movie_def_impl::add_abc(tu_string& name, AbcInfo* abc)
	{
		assert(m_abc == NULL);
		m_abc = abc;
		m_abc_name = name;

        Linker linker( get_player()->m_avm_domain, m_abc, get_player() );
        linker.link();
	}
...
//		fontlib::input_cached_data(in, fonts, this);
	...
	// flash9
	Function* movie_def_impl::instanciateClass(character* ch) const
	{
		...
		if (m_symbol_class.get(symbol_id, &class_name))
		{
            ...
            Class* cls = get_player()->m_avm_domain->findClassQualified( class_name );
            if( cls ) {
                ch->setType( cls );
                ch->setTraits( cls->instanceTraits() );
                cls->construct( ch );
            }
        }
		return NULL;
