...
namespace gameswf
{
	...
	void attach_video(const fn_call& fn)
	{
		...
		if (video && fn.nargs > 0)
		{
			...
			video->attach_netstream(cast_to<as_netstream>(fn.arg(0).asObject()));
		}
	}
	...
	void clear_video_background(const fn_call& fn)
	{
		...
		if (video && fn.nargs > 0)
		{
			...
			video->m_video_handler->clear_background(fn.arg(0).asBool());
		}
	}
		...
		m_def(def)
	{
		...
		builtin_member("clearBackgroundColor", Value(Value(), clear_video_background));
	}
