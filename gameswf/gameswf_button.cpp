...
namespace gameswf
{
	struct button_character_instance : public character
	{
			...
			m_enabled(true)
		{
			...
			for (r = 0; r < r_num; r++)
			{
				...
				ch->set_matrix(mat);
				ch->set_cxform(cx);
                ch->set_blend_mode(bdef->m_blend_mode); ///!!!Plarium
	...
	//			ch->on_event(event_id::CONSTRUCT);	// isn't tested
		...
		virtual bool get_topmost_mouse_entity( character * &te, float x, float y, bool shouldHandleMouse)
