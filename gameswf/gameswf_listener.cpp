...
namespace gameswf
{
		...
		for (int i = 0, n = listeners.size(); i < n; i++)
		{
			...
			if (listeners[i]->get_member(event_name, &function))
			{
				call_method(function, &env, listeners[i].get_ptr(),
					nargs, env.get_top_index(), event_name.c_str());
			}
		}
	}
	...
	void listener::add(Object* listener) 
	{
		// sanity check
	...
	bool listener::remove(Object* listener) 
	{
	...
	Object*	listener::operator[](const tu_stringi& name) const
	{
		return operator[](atoi(name.c_str()));
	}
	
	Object*	listener::operator[](int index) const
	{
