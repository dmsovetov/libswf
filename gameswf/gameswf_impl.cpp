...
namespace gameswf
{
	...
	void character_def::set_registered_class_constructor(const Value& value)
	{
		m_registered_class_constructor = value.asFunction();
	}
	...
	void character_def::instanciate_registered_class (character* ch)
	{
		...
		if (m_registered_class_constructor != NULL)
		{
			...
			ch->m_this_ptr = ch;
			Object* proto = ch->create_proto(m_registered_class_constructor.get_ptr());
			UNUSED(proto);
			...
			call_method(m_registered_class_constructor.get_ptr(), &env, ch, 0, 0, "ctor");
		}
    #else
        movie_def_impl* movieDef = cast_to<movie_def_impl>(this);

        if (movieDef == NULL)
        {
            sprite_definition* spriteDef = cast_to<sprite_definition>(this);

            if(spriteDef != NULL)
            {
                movieDef = cast_to<movie_def_impl>(spriteDef->m_movie_def);
            }
        }

        assert(movieDef);
        movieDef->instanciateClass(ch);
    #endif
	}
	...
	void	ensure_loaders_registered()
	{
		...
		if (s_tag_loaders_registered == false)
		{
			// Register the standard loaders.
			s_tag_loaders_registered = true;
...
//          register_tag_loader(16, stop_sound_loader);
...
//          register_tag_loader(23, define_definebuttoncxform);
			register_tag_loader(24, null_loader);           // "protect" tag; we're not an authoring tool so we don't care.
//          register_tag_loader(25, paths_are_postscript);	// PathsArePostscript | Define | The shape paths are defined as in postscript?
			register_tag_loader(26, place_object_2_loader);
			register_tag_loader(28, remove_object_2_loader);
//          register_tag_loader(29, SyncFrame);                     // | Display | Tag used to synchronize the animation with the hardware
//          register_tag_loader(31, FreeAll);                       // | Define  | Probably an action that would be used to clear everything out.
			register_tag_loader(32, define_shape_loader);           // | Define  | Define a simple geometric shape
			register_tag_loader(33, define_text_loader);            // | Define  | Defines a text of characters displayed using a font.
                                                                    //             Transparency is supported with this tag
			register_tag_loader(34, button_character_loader);       // | Define  | Define an action button. Includes a color transformation
			register_tag_loader(35, define_bits_jpeg3_loader);      // | Define  | Defines a complete JPEG image, i.e. a verbatim JPEG image file....
            register_tag_loader(36, define_bits_lossless_2_loader); // | Define  | Defines an RGBA bitmap compressed using ZLIB (similar to the PNG format).
			register_tag_loader(37, define_edit_text_loader);       // | Define  | An edit text enables the end users to enter text in a Flash window.
//          register_tag_loader(38, DefineVideo);                   // | Define  | Apparently, Macromedia did have a first attempt in supporting video on their platform.
                                                                    //             It looks, however, as if they reconsidered at that point in time.
			register_tag_loader(39, sprite_loader);                 // | Define  | Declares an animated character
//          register_tag_loader(40.	NameCharacter);                 // | Define  | Define the name of an object (for buttons, bitmaps, sprites and sounds.)
//          register_tag_loader(41.	ProductInfo);                   // | Define  | This tag defines information about the product used to generate the animation.
//          register_tag_loader(42.	DefineTextFormat);              // | Define  | Another tag that Flash ended up not using.
			register_tag_loader(43, frame_label_loader);            // | Define  | Names a frame or anchor. This frame can later be referenced using this name.
			register_tag_loader(45, sound_stream_head_loader);      // | Define  | Declare a sound effect which will be interleaved with  a movie data
			register_tag_loader(46, define_shape_morph_loader);     // | Define  | This is similar to a sprite with a simple morphing between two shapes.
//          register_tag_loader(47, GenerateFrame );                // | Define  | This may have been something similar to a New in an action script and thus was removed later.
			register_tag_loader(48, define_font_loader);            // | Define  |
            
//          register_tag_loader(49, GeneratorCommand);              // | Define  | Gives some information about the tool which generated this SWF file and its version
//          register_tag_loader(50, DefineCommandObject);           // | Define  | ?
//          register_tag_loader(51, CharacterSet);                  // | Define  | It looks like this would have been some sort of DefineSprite extension...
//          register_tag_loader(52,	ExternalFont);                  // | Define  | It looks like accessing a system font was going to be another tag,
			register_tag_loader(56, export_loader);                 // | Define  |
			register_tag_loader(57, import_loader);                 // | Define  |
			register_tag_loader(58, define_enable_debugger_loader); // | Format  |
			register_tag_loader(59, do_init_action_loader);         // | Action  |
			register_tag_loader(60, define_video_loader);           // | Define  |
			register_tag_loader(61, video_loader);                  // | Define  |
			register_tag_loader(62, define_font_info_loader);       // | Define  |
//          register_tag_loader(63, DebugID);                       // | Define  | This tag is used when debugging an SWF movie.
                                                                    //             It gives information about what debug file to load to match the
                                                                    //             SWF movie with the source. The identifier is a UUID.
			register_tag_loader(64, define_enable_debugger_loader); // | Format   |
			register_tag_loader(66, define_tabindex_loader);        // | Define   | Define the order index so the player knows where to go next when the Tab key is pressed by the user
			register_tag_loader(69, define_file_attribute_loader);	// | Format   | Flash 8
			register_tag_loader(70, place_object_2_loader);         // | Display  | Place an object in the display list. The object can include bitmap caching information, a blend mode and a set of filters.
            ...
            register_tag_loader(78, define_scaling_grid);           // DefineScalingGrid	///!!!Plarium
...
*/
	...
	void	define_abc_loader(stream* in, int tag_type, movie_definition_sub* m)
	{
		...
		AbcInfo* abc = new AbcInfo(m->get_player());
		abc->read(in, m);
		m->add_abc(name, abc);
	}
	...
	struct place_object_2 : public execute_tag
	{
		...
		place_object_2() :
			...
			m_character_id(0), m_clip_depth(0), m_blend_mode(0), m_place_type(PLACE), m_filter(NULL)
		{
		}
		...
		void	read(player* player, stream* in, int tag_type, int movie_version)
		{
			...
			if (tag_type == 26 || tag_type == 70)
			{
                ...
                if (has_class_name || (has_image && has_char))  ///!!!Plarium
                {
                    tu_string class_name;
                    in->read_string(&class_name);
                }
                ...
                if (has_cache_asbitmap) ///!!!Plarium
                {
                    int cache = in->read_u8();
                    IF_VERBOSE_ACTION(log_msg("   cache = %d\n", cache));
                }

                //if (has_visible)    ///!!!Plarium
                //{
                //    int visible = in->read_u8();
                //    rgba r;
                //    r.read_rgb(in);
                //    IF_VERBOSE_ACTION(log_msg("   visible = %d\n", visible));
                //}
	...
	void	export_loader(stream* in, int tag_type, movie_definition_sub* m) // Load an export tag (for exposing internal resources of m)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (font* f = m->get_font(id))
			{
                ...
                f->set_identifier(resource_name);	///!!!Plarium
			}
            ...
            else if(bitmap_character_def* bmp = m->get_bitmap_character(id)) ///!!!Plarium
            {
                bitmap_info* bi = bmp->get_bitmap_info();
                assert(bi);
                if( bi ) {
                    bi->set_identifier(resource_name);
                }
            }
			else
			{
...
//				m->get_loading_frame(), sample_count, data_size, seek_samples);
	...
	///!!!Plarium
    void define_scaling_grid(stream* in, int tag, movie_definition_sub* m)
    {
        assert(tag == 78);

        rect splitter;
        Uint16 ch = in->read_u16();
        splitter.read(in);

        character_def* def = m->get_character_def(ch);
        assert(def && (def->is(button_character_definition::m_class_id) || def->is(sprite_definition::m_class_id)));

        def->set_scaling_grid(new scaling_grid(splitter));
    }

	// Bitmap character implementation

	bitmap_character::bitmap_character(player* player, bitmap_info* bi) :
