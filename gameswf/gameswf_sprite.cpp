...
namespace gameswf
{
	...
	sprite_instance::sprite_instance(player* player, movie_definition_sub* def,	root* r, character* parent, int id) :
        ...
        m_is_constructed(false),    ///!!!Plarium
		m_script(NULL)
	{
		assert(m_def != NULL);
		assert(m_root != NULL);
		...
		m_init_tags_executed.resize(m_def->get_frame_count());
		memset(&m_init_tags_executed[0], 0,
               sizeof(m_init_tags_executed[0]) * m_init_tags_executed.size());

		set_ctor(as_global_movieclip_ctor);
    #endif

        m_root->add_mouse_listener(this);   ///!!!Plarium
	}
	...
	bool sprite_instance::get_topmost_mouse_entity( character * &top_ent, float x, float y, bool shouldHandleMouse ) ///!!!Plarium - shouldHandleMouse
	...
	// none.  Coords are in parent's frame.
	{
		...
		for (i = n - 1; i >= 0; i--)
		{
			...
			{
				if (ch->get_topmost_mouse_entity(te, p.m_x, p.m_y, shouldHandleMouse))  ///!!!Plarium - shouldHandleMouse
				{
					this_has_focus = true;
					// The containing entity that 1) is closest to root and 2) can
					// handle mouse events takes precedence.
					if (te && (te->can_handle_mouse_event() || shouldHandleMouse == false)) ///!!!Plarium - shouldHandleMouse == false
					{
						top_te = te;
						break;
					}
    ...
    void sprite_instance::construct()   ///!!!Plarium
    {
        if (m_is_constructed)
        {
            return;
        ...
        // ** Only for an ActionScript 2 engine
    ...
    void sprite_instance::invokeConstructor( void )
    {
        m_def->instanciate_registered_class(this);

    //    as_environment env(m_player.get_ptr());
    //    gameswf::call_method( ( Object* )m_class->init(), &env, this, 0, 0, "ctor");
    }
    ...
    #if ENABLE_AS2
        ///!!!Plarium
        if (m_goto_frame_action_list.size())
        {
            execute_postponed_actions(delta_time);
        }
        else
        {
...
#if ENABLE_AS2
    void sprite_instance::execute_init_tags(int frame)  ///!!!Plarium
    {
        // Execute this frame's init actions, if necessary.
        if (m_init_tags_executed[frame])
        {
            return;
        }
        ...
        m_init_actions.append(m_action_list);
        m_action_list.clear();

        // Mark this frame done, so we never execute these init actions
        // again.
        m_init_tags_executed[frame] = true;
    }
#endif
    ...
    #if ENABLE_AS2
        execute_init_tags(frame);   ///!!!Plarium
    #endif
...
#if ENABLE_AS2
    void    sprite_instance::execute_postponed_actions(float delta_time)    ///!!!Plarium
    {
        while (m_goto_frame_action_list.size())
        {
            m_display_list.advance(delta_time);
    ...
    void	sprite_instance::do_init_actions()  ///!!!Plarium
	{
        // Keep m_as_environment alive during any method calls!
		gc_ptr<Object>	this_ptr(this);

    #if ENABLE_AS2
		execute_actions(&m_as_environment, m_init_actions);
		m_init_actions.clear();
    #endif
	}
		...
		// Add an object to the display list.
	{
        ...
        cdef->instanciate_registered_class(ch); ///!!!Plarium
...
//		ch->on_event(event_id::CONSTRUCT);	// isn't tested
	...
	bool sprite_instance::set_member(const tu_string& name, const Value& val)
	{
        ...
        ///!!!Plarium
			...
			{
				Value new_val(val);
			//	call_watcher(name, Value(m_enabled), &new_val);
				m_enabled = new_val.asBool();
				return true;
			}
		}
	...
	bool sprite_instance::get_member(const tu_string& name, Value* val)
	{
	...
	void sprite_instance::call_frame_actions(const Value& frame_spec)
	...
	// at a new depth.
	{
		...
		{ 
            ...
            ch->advance(1);	///!!!Plarium - force advance
	...
	// Dispatch event handler(s), if any.
	{
		...
		if (get_member(method_name, &method))
		{
			...
			gameswf::call_method(method, &m_as_environment, this, nargs, 
				m_as_environment.get_top_index(), method_name.c_str());
        ...
        if( id == event_id::ENTER_FRAME ) {
            dispatchEvent( new Event( get_player(), "ENTER_FRAME" ) );
        }
	...
	tu_string sprite_instance::call_method(const char* method_name, Value * arguments, int argument_count )
	{
		return gameswf::call_method( get_environment(), this, method_name, arguments, argument_count, method_name );
	}
    ...
    /*
		...
		Value	obj = get_environment()->get_variable(tu_string(path_to_object), dummy);

		character*	m = cast_to<character>(obj.asObject());
		if (m)
		{
...
//		textfield->on_event(event_id::CONSTRUCT);	// isn't tested
	...
	void sprite_instance::clear_refs(hash<Object*, bool>* visited_objects, Object* this_ptr)
	{
		// Is it a reentrance ?
		...
		// will be set in Object::clear_refs
//		visited_objects->set(this, true);

		Object::clear_refs(visited_objects, this_ptr);
		...
		int depth)
	{
		...
		m_display_list.add_display_object( sprite, depth, true, m_color_transform, matrix::identity, 0.0f, 0, 0);   ///!!!Plarium
	...
	void sprite_instance::local_to_global(Object* obj)
	{
		Value x;
		obj->get_member("x", &x);
		Value y;
		obj->get_member("y", &y);
		if (x.isNumber() == false || y.isNumber() == false)
		{
			return;
		}

		point pt(x.asFloat(), y.asFloat());
		pt.pixels_to_twips();
		point result;
	...
	void sprite_instance::global_to_local(Object* obj)
	{
		Value x;
		obj->get_member("x", &x);
		Value y;
	...
	// flash9
	void sprite_instance::add_script(int frame, Function* func)
	// frame is 0-based
	{
		if (frame >= 0 && frame < m_def->get_frame_count())
		{
			if (m_script == NULL)
			{
				m_script = new hash<int, gc_ptr<Function> >;
			}
