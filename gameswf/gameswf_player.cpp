...
namespace gameswf
{
    ...
    static file_download_callback s_download_function = NULL;   ///!!!Plarium
    ...
    void    register_file_download_callback(file_download_callback callback)    ///!!!Plarium
    {
        s_download_function = callback;
    }

    file_opener_callback get_file_opener_callback() ///!!!Plarium
    {
        return s_opener_function;
    }

    file_download_callback get_file_download_callback() ///!!!Plarium
    {
        return s_download_function;
    }
	...
	static string_hash<Value>*	s_standard_method_map[BUILTIN_COUNT];
	...
	string_hash<Value>* new_standard_method_map(builtin_object id)
	{
		...
		{
			s_standard_method_map[id] = new string_hash<Value>;
		}
	...
	void standard_method_map_init()
	{
		...
		string_hash<Value>* map;

		// Object builtins
		map = new_standard_method_map(BUILTIN_OBJECT_METHOD);
	//	map->add("addProperty", Object_addproperty);
	//	map->add("registerClass", Object_registerclass);
	//	map->add("hasOwnProperty", Object_hasownproperty);
	//	map->add("watch", Object_watch);
	//	map->add("unwatch", Object_unwatch);

		// flash9
	//	map->add("addEventListener", Object_add_event_listener); ///!!!Plarium

		// for debugging
#ifdef _DEBUG
//		map->add("dump", Object_dump);
#endif
		...
		map->add("length", Value(string_length, Value()));
        ...
        // edit_text builtins
        map = new_standard_method_map(BUILTIN_TEXT_METHOD);
        map->add("setTextFormat", as_global_set_text_format);
		map->add("getTextFormat", as_global_get_text_format);
	}
	...
	Value	get_property(Object* obj, int prop_number)
	{
		Value	val;
	...
	void	set_property(Object* obj, int prop_number, const Value& val)
	{
		if (prop_number >= 0 && prop_number < int(sizeof(s_property_names)/sizeof(s_property_names[0])))
		{
	...
	{
	//	m_global = new Object(this);

	//	action_init();
        m_avm_domain = new FlashDomain( this );
        m_avm_domain->registerPackages();
...
/*
	...
	//	m_global->builtin_member("MovieClip", as_global_movieclip_ctor);
		...
		//			m_global->set_member("XML", Value(xml_new));
		...
		Object * capabilities = new Object(this);
		...
		m_global->builtin_member("$version", Value(as_global_get_version, Value()));
...
*/
    void  player::register_native_function(const char* identifier, FunctionNativeThunk callback, void* userData ) ///!!!Plarium
    {
        assert(false);
    //    m_global->set_member(identifier, Value(callback, userData) );
    }

//	Object* player::get_global() const
//	{
//		return m_global.get_ptr();
//	}

    Domain* player::get_domain()
    {
        return m_avm_domain;
    }
	...
	void player::notify_key_object(key::code k, bool down)
	{
        ...
        assert(false);
	//	Object* global = get_global();

	//	global->get_member("Key", &kval);
		as_key*	ko = cast_to<as_key>(kval.asObject());
		if (ko)
		{
...
//		assert(m_current_root.get_ptr() != NULL);
	...
	// can be cleaned up.
	{
			...
			m_chardef_library.begin(); it != m_chardef_library.end(); ++it)
		{
			...
			{
				IF_VERBOSE_DEBUG(printf("memory leaks is found out: on exit movie_definition_sub ref_count > 1\n"));
				IF_VERBOSE_DEBUG(printf("this = 0x%p, ref_count = %d\n", it->second.get_ptr(), gc_collector::debug_get_ref_count(it->second)));
    ...
    bool player::is_valid_movie(const char *filename) const ///!!!Plarium
    {
        tu_file* in = s_opener_function(filename);
        ...
        in->set_position(file_start_pos);
	...
	void player::set_alive(Object* obj)
	{
		m_heap.set(obj, false);
	}

	bool player::is_garbage(Object* obj)
	{
	...
	{
		for (hash<gc_ptr<Object>, bool>::iterator it = m_heap.begin();
			it != m_heap.end(); ++it)
		{
			Object* obj = it->first.get_ptr();
			if (obj)
			{
				if (gc_collector::debug_get_ref_count(obj) > 1)
				{
					hash<Object*, bool> visited_objects;
