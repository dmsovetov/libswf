...
namespace gameswf
{
	...
	// then arg1 is at env->bottom(7), arg2 is at env->bottom(6), etc.
	{
        ...
        return Value::undefined;
	//	Value	val;
	//	Function*	func = method.asFunction();
	//	if (func)
	//	{
	//		// It's a function.  Call it.
	//		(*func)(fn_call(&val, this_ptr, env, nargs, first_arg_bottom_index, name));
    ...
    /*
		...
		Value	method = env->get_variable(method_name, dummy_with_stack);
		...
		// Do the call.
		Value	result = call_method(method, env, this_ptr, nargs, env->get_top_index(), method_name);
		env->drop(nargs);

		// Return pointer to static string for return value.
		static tu_string	s_retval;
		s_retval = result.asString();
    ...
    */
    ...
    /*
		...
		// Do the call.
		Value	result = call_method(method, env, this_ptr, nargs, env->get_top_index(), name);
		env->drop(nargs);
...
//		env->set_local_frame_top(local_stack_top);
	...
	// static
	void action_buffer::enumerate(as_environment* env, Object* obj)
	{
		assert(env);
	...
	Object* action_buffer::load_as_plugin(player* player, const tu_string& classname,
		...
		for (int pc = start_pc; pc < stop_pc; )
		{
			...
			if ((action_id & 0x80) == 0)
			{
				...
				case 0x1C:	// get variable
				{
					...
					Value variable = env->get_variable(var_string, with_stack);
					env->top(0) = variable;
				...
				// duplicateMovieClip(target:MovieClip, newname:String, depth:Number) : Void
				{
					...
					if (target == NULL)
					{
						Value val = env->get_variable(env->top(2).to_string(), with_stack);
				...
				{
					// Log the stack val.
					as_global_trace(fn_call(&env->top(0), Value(), env, 1, env->get_top_index(), "trace"));
					env->drop(1);
					break;
				}
				...
				case 0x2B:	// cast_object
				{
					...
					Object* obj = env->top(0).to_object();
				...
				case 0x3A:	// delete
				{
					...
					Object* obj_interface = env->top(1).to_object();
					env->drop(1);
					...
					if (obj_interface)
					{
						...
						{
							obj_interface->set_member(varname, Value());
							retcode = true;
						}
					}
				...
				case 0x3B:	// delete2
				{
					...
					Value obj(env->get_variable_raw(varname, with_stack));
				...
				{
					Value	function;
                    Value    this_ptr( (Object*)NULL );
                    const char* name = "unknown";   ///!!!Plarium
					...
					if (env->top(0).is_string())
					{
                        ...
                        name = function_name.c_str();   ///!!!Plarium

						// super constructor, Flash 6 
						if (function.is_object())
						{
							Object* obj = function.to_object();
							if (obj)
							{
					...
					Value	result = call_method(function, env, this_ptr, nargs, env->get_top_index() - 2, name);
					env->drop(nargs + 1);
					env->top(0) = result;
					break;
				}
				...
				case 0x40:	// new
				{
					Value	classname = env->pop();
					IF_VERBOSE_ACTION(log_msg("-------------- new object: %s\n",
								  classname.to_tu_string().c_str()));
					int	nargs = env->pop().to_int();
					Value constructor = env->get_variable(classname.to_tu_string(), with_stack);
					Value new_obj;
					...
					if (as_c_function* c_constructor = cast_to<as_c_function>(constructor.to_object()))
					{
						...
						(*c_constructor)(fn_call(&new_obj, Value(), env, nargs, env->get_top_index(), "new"));
					}
					else if (as_s_function* s_constructor = cast_to<as_s_function>(constructor.to_object()))
					{

						// Create an empty object
						gc_ptr<Object>	new_obj_ptr = new Object(env->get_player());
						Object* proto = new_obj_ptr->create_proto(s_constructor);
						...
						call_method(s_constructor, env, new_obj_ptr.get_ptr(), nargs, env->get_top_index(), "new");
				
						new_obj_ptr->set_ctor(s_constructor);
						new_obj.set_Object(new_obj_ptr.get_ptr());
					}
					else
					{
						...
						Object* plugin = load_as_plugin(env->get_player(), classname.to_tu_string(), params);
						if (plugin)
						{
							new_obj.set_Object(plugin);
						}
						else
						{
				...
				case 0x42:	// init array
				{
 					...
 					Value	result;
 					as_global_array_ctor(fn_call(&result, Value(), env, -1, -1, "array"));
 					env->push(result);
					break;
				}
				...
				// property is converted to a string. The value may be of any type.				{
					...
					Object* obj = new Object(env->get_player());
...
//						obj->builtin_member(env->top(1).to_tu_string(), env->top(0));
				...
				case 0x46:	// enumerate
				{
					Value variable = env->get_variable(env->top(0).to_tu_string(), with_stack);
				...
				{
					// keep alive 'obj'
					gc_ptr<Object>	obj = env->top(1).to_object();
					if (obj == NULL)
					{
					...
					else
					{
						...
						if (obj->get_member(env->top(0).to_tu_string(), &(env->top(1))) == false)
						{
							...
							if (obj->get_member("__resolve", &val))
							{
								...
								if (resolve)
								{
									(*resolve)(fn_call(&val, obj.get(), env, 1, env->get_top_index(), "__resolve"));
									env->top(1) = val;
								}
							}
						}
				...
				{
					Object*	obj = env->top(2).to_object();
					if (obj)
					{
						obj->set_member(env->top(1).to_tu_string(), env->top(0));
						IF_VERBOSE_ACTION(
...
//								env->top(2).to_tu_string().c_str(),
				...
				case 0x52:	// call method
				{
					...
					if (method_name == "undefined" || method_name == "")
					{
						...
						Object* obj = env->top(1).to_object();
						...
						else if (obj && obj->m_this_ptr != NULL)
						{
							...
							{
							//	obj->create_proto(constructor); ///!!!Plarium - probably we don't need this any more, should be called inside character_def::instanciate_registered_class
								result = call_method(
									constructor,
									env,
									obj,
...
//#endif
				...
				// The �undefined� return value should be popped off the stack.
					...
					Value	constructor = env->pop();
					Object*	obj = env->pop().to_object();
					int	nargs = env->pop().to_int();

					// Create an empty object
					gc_ptr<Object>	new_obj;

					if (cast_to<as_c_function>(constructor.to_object()))
					{
						// C function is responsible for creating the new object and setting members.
						new_obj = call_method( constructor, env, Value(), nargs, env->get_top_index(), "newmethod").to_object();
					}
					else if (cast_to<as_s_function>(constructor.to_object()))
					{
						new_obj = new Object(env->get_player());
						Object* proto = new_obj->create_proto(constructor);
						proto->m_this_ptr = new_obj.get_ptr();

						// Call the actual constructor function; new_obj is its 'this'.
						// We don't need the function result.
						call_method(constructor, env, new_obj.get_ptr(), nargs, env->get_top_index(), "newmethod");
					}
					else
					{
						Value val;
...
//								prototype.to_object()->set_member("__constructor__", func);
								...
								new_obj = new Object(env->get_player());
								Object* proto = new_obj->create_proto(func);
								proto->m_this_ptr = new_obj.get_ptr();

								// Call the actual constructor function; new_obj is its 'this'.
								// We don't need the function result.
								call_method(func, env, new_obj.get_ptr(), nargs, env->get_top_index(), "newmethod");
							}
							else if( as_c_function* c_func = cast_to<as_c_function>(val.to_object()) )
							{
								...
								new_obj = call_method(c_func, env, Value(), nargs, env->get_top_index(), "newmethod").to_object();
							}
							else
							{
			...
			else
			{
				...
				case 0x83:	// get url
				{
					...
					{
						env->load_file(url, Value(target));
					}
				...
				case 0x8B:	// set target
				{
					...
					Value val = (const char*) &buffer[pc + 3];
				...
				case 0x94:	// with
				{
					...
					if (with_stack.size() < 8) //todo, depends on flash version, could be 16
					{
 						...
 						Object*	with_obj = env->top(0).to_object();
...
//							}
	...
	static tu_string	s_function_names[event_id::EVENT_COUNT] =
	{
		...
		"onPress",		 // PRESS
		"onRelease",		 // RELEASE
		"onReleaseOutside",	 // RELEASE_OUTSIDE ///!!!Plarium - onRelease_Outside -> onReleaseOutside
