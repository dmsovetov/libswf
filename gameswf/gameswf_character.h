...
namespace gameswf
{
	...
	///!!!Plarium
    struct scaling_grid : public ref_counted
    {
        enum { CENTER, LEFT_TOP, CENTER_TOP, RIGHT_TOP, RIGHT_CENTER, RIGHT_BOTTOM, CENTER_BOTTOM, LEFT_BOTTOM, LEFT_CENTER };
        enum { LEFT, RIGHT, TOP, BOTTOM };
        
        rect                    m_splitter;
        float                   m_width, m_height;
        rect                    m_bounds[9];
        float                   m_borderSize[4];

                                scaling_grid(const rect& splitter)
                                    : m_splitter(splitter) {}

        void                    calculate();
        void                    calculate_slice(int index, float& x, float& y, float& scaleX, float& scaleY) const;
        point                   calculate_size(float scaleX, float scaleY) const;
        float                   get_vertical_border_size() const;
        float                   get_horizontal_border_size() const;
        float                   get_slice_width(int index) const;
        float                   get_slice_height(int index) const;
        float                   get_border_size(int index) const;

    private:

        float                   calculate_border_width(int border) const;
        float                   calculate_border_height(int border) const;
        void                    calculate_scale(int index, const point& size, float& scaleX, float& scaleY) const;
        point                   calculate_offset(int index, const point& size) const;
    };
	...
	struct character_def : public Object_interface
	{
		...
		void set_registered_class_constructor(const Value & value);
		void instanciate_registered_class(character * ch);
		player* get_player() const { return m_player.get_ptr(); }

		///!!!Plarium
        scaling_grid* get_scaling_grid() const { return m_scaling_grid.get_ptr(); }
        void set_scaling_grid(scaling_grid * value) { m_scaling_grid = value; }

		// flash9
		virtual Function* instanciateClass(character* ch) const { return NULL; }

		private:

		int	m_id;
		weak_ptr<Function> m_registered_class_constructor;
		weak_ptr<player> m_player;
        gc_ptr<scaling_grid> m_scaling_grid;	///!!!Plarium
	};
	...
	// internal interface
	struct character : public EventDispatcher
	{
		...
		virtual bool is(int class_id) const
		{
			...
			else return EventDispatcher::is(class_id);
		}
		...
		//
		virtual bool get_topmost_mouse_entity( character * &te, float x, float y, bool shouldHandleMouse = true) { return false; }  ///!!!Plarium - shouldHandleMouse
...
//#endif	// not __GNUC__
		...
		virtual tu_string	call_method(const char* method_name, Value * arguments, int argument_count )
			// Override this if you implement call_method.
		{
			assert(0);
			return tu_string();
        ...
        virtual bool on_event( event_id id ) { assert(false); return false; }
        ...
        virtual player* get_player() { return m_player.get(); }
        virtual root*   get_root() { return m_player->get_root(); }
//		virtual character*	get_root_interface() { return NULL; }
		virtual void	call_frame_actions(const Value& frame_spec) { assert(0); }
        ...
        virtual void    construct() {} ///!!!Plarium
		...
		virtual bool	set_member(const tu_string& name, const Value& val);
		...
		bool is_alive() const { return m_depth > 0 || (m_parent == NULL && m_id == -1); }   ///!!!Plarium
	};
