...
namespace gameswf
{
	...
	void	sprite_hit_test(const fn_call& fn)
	{
		...
		if (fn.nargs == 1) 
		{
			...
			if (ch)
			{
				fn.result->setBool(sprite->hit_test(ch));
				return;
			}
		...
		{
			fn.result->setBool(sprite->hit_test( fn.arg(0).asNumber(), fn.arg(1).asNumber(), fn.arg(2).asBool() ) );
		}
	}
	...
	void	sprite_start_drag(const fn_call& fn)
	{
		...
		if (fn.nargs > 0)
		{
			...
			ds.SetLockCentered(fn.arg(0).asBool());

			if (fn.nargs >= 5)
			{
				// bounds have been given
				tu_float x0 = PIXELS_TO_TWIPS(fn.arg(1).asFloat());
				tu_float y0 = PIXELS_TO_TWIPS(fn.arg(2).asFloat());
				tu_float x1 = PIXELS_TO_TWIPS(fn.arg(3).asFloat());
				tu_float y1 = PIXELS_TO_TWIPS(fn.arg(4).asFloat());
	...
	void	sprite_goto_and_play(const fn_call& fn)
	{
		...
		if (fn.arg(0).isString() || fn.arg(0).isNumber() || fn.arg(0).isProperty())
		{
			sprite->goto_frame(fn.arg(0).asString());
			sprite->set_play_state(character::PLAY);
		}
	}
	...
	{
		sprite_instance* sprite = sprite_getptr(fn);
		fn.result->setInt(sprite->get_loaded_bytes());
	}

	void	sprite_get_bytes_total(const fn_call& fn)
	{
		sprite_instance* sprite = sprite_getptr(fn);
		fn.result->setInt(sprite->get_file_bytes());
	}
	...
	{ 
		...
		if (fn.arg(0).isNumber())
		{ 
			int target_depth = fn.arg(0).asInt();
		...
		{ 
			target = cast_to<sprite_instance>(fn.arg(0).asObject());
		}
		else 
		{ 
	...
	{ 
		...
		{ 
			character* ch = sprite->clone_display_object(
				fn.arg(0).asString(),
				fn.arg(1).asInt() + ADJUST_DEPTH_VALUE);

			if (fn.nargs == 3) 
			{ 
				Object* init_object = fn.arg(2).asObject();
				if (init_object)
				{
	...
	{
		sprite_instance* sprite = sprite_getptr(fn);
		fn.result->setInt(sprite->get_depth() - ADJUST_DEPTH_VALUE);
	}
	...
	void sprite_create_empty_movieclip(const fn_call& fn)
	{
		...
		character* ch = sprite->add_empty_movieclip(fn.arg(0).asString(),
			fn.arg(1).asInt() + ADJUST_DEPTH_VALUE);
		fn.result->setObject(ch);
	}
	...
	{ 
		if (fn.nargs >= 1)
		{
			character* mc = fn.env->load_file(fn.arg(0).asString(), fn.this_ptr);

			// extension
			fn.result->setObject(mc);
		}
...
//				UNUSED(sprite);
	...
	{ 
		...
		fn.result->setInt(highest_depth);
	} 
	...
	{ 
		...
		fn.result->setObject(sprite->create_text_field(
			fn.arg(0).asCString(),	// field name
			fn.arg(1).asInt() + ADJUST_DEPTH_VALUE,	// depth
			fn.arg(2).asInt(),	// x
			fn.arg(3).asInt(),	// y
			fn.arg(4).asInt(),	// width
			fn.arg(5).asInt()	// height
		));
	} 
	...
	{ 
		...
		if (fn.nargs >= 3)
		{
			tu_string id = fn.arg(0).asString();	// the exported name (sprite_definition)
			tu_string name = fn.arg(1).asString();	// instance name
			int depth = fn.arg(2).asInt() + ADJUST_DEPTH_VALUE;
			sprite_instance* ch = sprite->attach_movie(id, name, depth);

			if (fn.nargs >= 4)
			{
				Object* init_object = fn.arg(3).asObject();
				if (init_object)
				{
					init_object->copy_to(ch);
				}
	...
	void sprite_move_to(const fn_call& fn)
	{
		...
		if (fn.nargs >= 2)
		{
			float x = PIXELS_TO_TWIPS(fn.arg(0).asFloat());
			float y = PIXELS_TO_TWIPS(fn.arg(1).asFloat());
			canva->move_to(x, y);
		}
	}
	...
	void sprite_curve_to(const fn_call& fn)
	{
		...
		if (fn.nargs >= 4)
		{
			float cx = PIXELS_TO_TWIPS(fn.arg(0).asFloat());
			float cy = PIXELS_TO_TWIPS(fn.arg(1).asFloat());
			float ax = PIXELS_TO_TWIPS(fn.arg(2).asFloat());
			float ay = PIXELS_TO_TWIPS(fn.arg(3).asFloat());
	...
	void sprite_line_style(const fn_call& fn)
	{
		...
		Uint16 width = (Uint16) PIXELS_TO_TWIPS(fclamp(fn.arg(0).asFloat(), 0, 255));
		rgba color(0, 0, 0, 255);

		if (fn.nargs >= 2)
		{
			color.set(fn.arg(1).asFloat());
			if (fn.nargs >= 3)
			{
				float alpha = fclamp(fn.arg(2).asFloat(), 0, 100);
	...
	// flash9
	void sprite_add_script(Frame* frame)
	{
		sprite_instance* sprite = cast_to<sprite_instance>( frame->instance() );
		if (frame->nargs() == 2)
		{
			// arg #1 - frame number, 0 based
			// arg #2 - function
			sprite->add_script(frame->arg(0).asInt(), frame->arg(1).asFunction());
		}
	}
	...
	void	sprite_get_bounds(const fn_call& fn)
	{
		...
		if (fn.nargs > 0) 
		{
			target = cast_to<character>(fn.arg(0).asObject());
		}
		...
		Object* obj = new Object(sprite->get_player());
	...
	void	sprite_set_mask(const fn_call& fn)
	{
		...
		if (fn.nargs > 0)
		{
			...
			sprite->m_mask_clip = cast_to<sprite_instance>(fn.arg(0).asObject());
