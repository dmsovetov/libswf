...
namespace gameswf
{
	...
	struct listener : public ref_counted
	{
		...
		// misc
		struct clone_listener : public array<gc_ptr<Object> >
		{
			clone_listener(const array< weak_ptr<Object> >& listeners)
			{
		...
		exported_module void add(Object* listener);
		exported_module bool remove(Object* listener);
		...
		exported_module Object*	operator[](const tu_stringi& name) const;
		exported_module Object*	operator[](int index) const;
		exported_module int	size() const;
		exported_module void enumerate(as_environment* env) const;

	private:

		array< weak_ptr<Object> > m_listeners;
