...
namespace gameswf
{
	...
	static void audio_streamer(Object* netstream, Uint8* stream, int len)
	{
...
//			printf("pop status: %s %s\n", ev.level.c_str(), ev.code.c_str());		
			...
			{
				gc_ptr<Object> infoObject = new Object(get_player());
...
//			log_error("Couldn't open file '%s'\n", c_url);
	...
	{
		Object* netstream = new as_netstream(fn.get_player());

		// properties
		netstream->builtin_member("time", Value(netstream_time, Value()));
		netstream->builtin_member("bufferLength", Value(netstream_buffer_length, Value()));
		netstream->builtin_member("bufferTime", Value(netstream_buffer_time, Value()));
		...
		netstream->builtin_member("_width", Value(netstream_video_width, Value()));
		netstream->builtin_member("_height", Value(netstream_video_height, Value()));

		fn.result->set_Object(netstream);
	}
