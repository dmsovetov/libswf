...
namespace gameswf
{
	...
	void	sound_start(const fn_call& fn)
	{
		...
		if (s != NULL)
		{
			...
			if (snd)
			{
				...
				if (fn.nargs >= 2)
				{
					offset = fn.arg(0).asInt();
					loops = fn.arg(1).asInt();
				}
	...
	void	sound_attach(const fn_call& fn)
	{
		...
		if (target)
		{
			res = target->find_exported_resource(fn.arg(0).asString());
		}
	...
	void	as_global_sound_ctor(const fn_call& fn)
	{
		...
		snd->builtin_member("position", Value(get_position, Value()));

		fn.result->setObject(snd.get_ptr());
	}
