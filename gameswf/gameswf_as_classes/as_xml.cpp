...
array<Object *> _xmlobjs;    // FIXME: hack alert
...
XML::on_event(event_id id)
{
  ...
  //  gc_ptr<Object_interface>	this_ptr(this);
...
XML::extractNode(xmlNodePtr node, bool mem)
{
  ...
  while (childnode != NULL) {
    if (childnode->type == XML_ELEMENT_NODE) {
      ...
      //if (child->_value.get_type() != Value::UNDEFINED) {
      if (child->_value != 0) {
...
Object *
XML::setupFrame(Object *obj, XMLNode *xml, bool mem)
{
  int           child, i;
  const char    *nodename;
  ...
  Value      inum;
  XMLNode       *childnode;
  xmlnode_Object *xmlchildnode_obj;
  xmlattr_Object* attr_obj;
  ...
  if (xml->_value != 0) {
    ...
    obj->set_member("nodeValue", Value::UNDEFINED);
  }

//   if (nodevalue.get_type() != Value::UNDEFINED) {
...
//                xml->_attributes[i]->_value, nodename, obj);
  ...
  if (length) {
    ...
    for (child=0; child<length; child++) {
      ...
      xmlchildnode_obj = new xmlnode_Object;
...
xml_load(const fn_call& fn)
{
  ...
  xml_Object *xml_obj = (xml_Object*)fn.this_ptr;
...
xml_onload(const fn_call& fn)
{
  ...
  xml_Object*	ptr = (xml_Object*) (Object*) fn.this_ptr;
  assert(ptr);
...
xml_ondata(const fn_call& fn)
{
  ...
  xml_Object*	ptr = (xml_Object*)fn.this_ptr;
  assert(ptr);
...
xml_new(const fn_call& fn)
{
  ...
  if (fn.nargs > 0) {
    if (fn.env->top(0).get_type() == Value::STRING) {
      xml_obj = new xml_Object;
      ...
      xml_Object*	xml_obj = (xml_Object*)fn.env->top(0).to_object();
      //log_msg("\tCloned the XML object at %p\n", xml_obj);
      //result->set(xml_obj);
      fn.result->set_Object_interface(xml_obj);
      return;
    }
  } else {
    xml_obj = new xml_Object;
