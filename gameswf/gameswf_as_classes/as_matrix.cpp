...
namespace gameswf
{
	...
	void	as_global_matrix_ctor(const fn_call& fn)
	{
		...
		case 6:
			matrix->m_matrix.m_[1][2] = fn.arg(5).asFloat();

		case 5:
			matrix->m_matrix.m_[0][2] = fn.arg(4).asFloat();

		case 4:
			matrix->m_matrix.m_[1][1] = fn.arg(3).asFloat();

		case 3:
			matrix->m_matrix.m_[1][0] = fn.arg(2).asFloat();

		case 2:
			matrix->m_matrix.m_[0][1] = fn.arg(1).asFloat();

		case 1:
			matrix->m_matrix.m_[0][0] = fn.arg(0).asFloat();

		case 0:
			break;
		}
		fn.result->setObject(matrix.get_ptr());
	}
	...
	void	as_matrix_translate(const fn_call& fn)
	{
		...
		temp.concatenate_translation(fn.arg(0).asFloat(), fn.arg(1).asFloat());
	...
	void	as_matrix_rotate(const fn_call& fn)
	{
		...
		rotation_matrix.set_scale_rotation(1, 1, fn.arg(0).asFloat());
	...
	void	as_matrix_scale(const fn_call& fn)
	{
		...
		scale_matrix.set_scale_rotation(fn.arg(0).asFloat(), fn.arg(1).asFloat(), 0);
	...
	void	as_matrix_concat(const fn_call& fn)
	{
		...
		as_matrix* other_matrix = cast_to<as_matrix>(fn.arg(0).asObject());
		if (other_matrix)
		{
			gameswf::matrix
				temp;
	...
	void as_matrix_clone(const fn_call& fn)
	{
		...
		fn.result->setObject(result.get_ptr());
	}
	...
	void as_matrix_transformPoint(const fn_call& fn)
	{
		...
		as_point* point = cast_to<as_point>(fn.arg(0).asObject());
		if (point)
		{
