...
namespace gameswf
{
	...
	void	as_global_color_ctor(const fn_call& fn)
	{
		if (fn.nargs == 1)
		{
			character* target = cast_to<character>(fn.arg(0).asObject());
			if (target)
			{
				fn.result->setObject(new as_color(fn.get_player(), target));
			}
		}
	}
	...
	void	as_color_gettransform(const fn_call& fn)
	{
		...
		Object* tobj = new Object(fn.get_player());
	...
	void	as_color_settransform(const fn_call& fn)
	{
		...
		if (tobj)
		{
			...
			{
				cx.m_[0][0] *= v.asFloat() / 100.0f;
			}
			else
			if (tobj->get_member("rb", &v))
			{
				cx.m_[0][0] = v.asFloat() / 255.0f;
			}

			if (tobj->get_member("ga", &v))
			{
				cx.m_[1][0] *= v.asFloat() / 100.0f;
			}
			else
			if (tobj->get_member("gb", &v))
			{
				cx.m_[1][0] = v.asFloat() / 255.0f;
			}

			if (tobj->get_member("ba", &v))
			{
				cx.m_[2][0] *= v.asFloat() / 100.0f;
			}
			else
			if (tobj->get_member("bb", &v))
			{
				cx.m_[2][0] = v.asFloat() / 255.0f;
			}

			if (tobj->get_member("aa", &v))
			{
				cx.m_[3][0] *= v.asFloat() / 100.0f;
			}
			else
			if (tobj->get_member("ab", &v))
			{
				cx.m_[3][0] = v.asFloat() / 255.0f;
			}
