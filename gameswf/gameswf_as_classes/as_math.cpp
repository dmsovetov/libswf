...
namespace gameswf
{
	...
	void	math_##funcname(Value* result, Object_interface* this_ptr,		\
				as_environment* env, int nargs, int first_arg_bottom_index)	\
	{											\
		double	arg = env->bottom(first_arg_bottom_index).asNumber();			\
		result->setNumber(funcname(arg));						\
	}
#else
	// One-argument simple functions.
	#define MATH_WRAP_FUNC1(funcname)							\
	...
	{											\
		double	arg = fn.arg(0).asNumber();						\
		fn.result->setNumber(funcname(arg));						\
	}
#endif
	...
	void	math_##funcname(Value* result, Object_interface* this_ptr, as_environment* env, int nargs, int first_arg_bottom_index)	\
	{															\
		double	arg0 = env->bottom(first_arg_bottom_index).asNumber();							\
		double	arg1 = env->bottom(first_arg_bottom_index - 1).asNumber();						\
		result->setNumber(expr);											\
	}
#else
	// Two-argument functions.
	#define MATH_WRAP_FUNC2_EXP(funcname, expr)										\
	void	math_##funcname(const fn_call& fn)										\
	{															\
		double	arg0 = fn.arg(0).asNumber();										\
		double	arg1 = fn.arg(1).asNumber();										\
		fn.result->setNumber(expr);											\
	}
#endif
	...
	void	math_random(const fn_call& fn)
	{
		...
		fn.result->setNumber(tu_random::next_random() / double(Uint32(0x0FFFFFFFF)));
	}
	void	math_round(const fn_call& fn)
	{
		// round argument to nearest int.
		double	arg0 = fn.arg(0).asNumber();
		fn.result->setNumber(floor(arg0 + 0.5));
	}
	
	Object* math_init(player* player)
	{
		// Create built-in math object.
		Object*	math_obj = new Object(player);
