...
namespace gameswf
{
	...
	{
		fn.result->set_Object(new as_xmlsock(fn.get_player()));
	}
	...
	bool as_xmlsock::connect(const char* host, int port)
	{
		...
		if (get_member("onConnect", &function))
		{
			...
			call_method(function, &env, Value(), 1, env.get_top_index());
		}
	...
	void as_xmlsock::send(const Value& val) const
	{
		if (m_ns)
		{
	...
	void	as_xmlsock::advance(float delta_time)
	{
		...
		if (m_ns->is_readable())
		{
			...
			if (str.size() == 0)
			{
				...
				{
					as_environment env(get_player());
					call_method(function, &env, Value(), 0, env.get_top_index());
				}
			}
			else
			{
