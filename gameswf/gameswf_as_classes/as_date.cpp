...
namespace gameswf
{
	...
	void	as_date_gettime(const fn_call& fn)
	{
		...
		fn.result->setNumber(dt->get_time());
	}
	...
	void	as_date_getdate(const fn_call& fn)
	{
		...
		fn.result->setInt(dt->get(as_date::MDAY));
	}
	...
	void	as_date_setdate(const fn_call& fn)
	{
		...
		if (fn.nargs > 0)
		{
			dt->set(as_date::MDAY, fn.arg(0).asInt());
			fn.result->setNumber(dt->get_time());
		}
	}
	...
	void	as_date_getday(const fn_call& fn)
	{
		...
		fn.result->setInt(dt->get(as_date::WDAY));
	}
	...
	void	as_date_setday(const fn_call& fn)
	{
		...
		if (fn.nargs > 0)
		{
			dt->set(as_date::WDAY, fn.arg(0).asInt());
			fn.result->setNumber(dt->get_time());
		}
	}
	...
	void	as_date_getfullyear(const fn_call& fn)
	{
		...
		fn.result->setInt(dt->get(as_date::FULLYEAR));
	}
	...
	void	as_date_setfullyear(const fn_call& fn)
	{
		...
		if (fn.nargs > 0)
		{
			dt->set(as_date::FULLYEAR, fn.arg(0).asInt());
			fn.result->setNumber(dt->get_time());
		}
	}
	...
	void	as_date_gethours(const fn_call& fn)
	{
		...
		fn.result->setInt(dt->get(as_date::HOUR));
	}
	...
	void	as_date_sethours(const fn_call& fn)
	{
		...
		if (fn.nargs > 0)
		{
			dt->set(as_date::HOUR, fn.arg(0).asInt());
			fn.result->setNumber(dt->get_time());
		}
	}
	...
	void	as_date_getminutes(const fn_call& fn)
	{
		...
		fn.result->setInt(dt->get(as_date::MIN));
	}
	...
	void	as_date_setminutes(const fn_call& fn)
	{
		...
		if (fn.nargs > 0)
		{
			dt->set(as_date::MIN, fn.arg(0).asInt());
			fn.result->setNumber(dt->get_time());
		}
	}
	...
	void	as_date_getmonth(const fn_call& fn)
	{
		...
		fn.result->setInt(dt->get(as_date::MON));
	}
	...
	void	as_date_setmonth(const fn_call& fn)
	{
		...
		if (fn.nargs > 0)
		{
			dt->set(as_date::MON, fn.arg(0).asInt());
			fn.result->setNumber(dt->get_time());
		}
	}
	...
	void	as_date_getseconds(const fn_call& fn)
	{
		...
		fn.result->setInt(dt->get(as_date::SEC));
	}
	...
	void	as_date_setseconds(const fn_call& fn)
	{
		...
		if (fn.nargs > 0)
		{
			dt->set(as_date::SEC, fn.arg(0).asInt());
			fn.result->setNumber(dt->get_time());
		}
	}
	...
	void	as_date_getyear(const fn_call& fn)
	{
		...
		fn.result->setInt(dt->get(as_date::YEAR));
	}
	...
	void	as_date_setyear(const fn_call& fn)
	{
		...
		if (fn.nargs > 0)
		{
			dt->set(as_date::YEAR, fn.arg(0).asInt());
			fn.result->setNumber(dt->get_time());
		}
	}
	...
	as_date::as_date(const fn_call& fn) :
		Object(fn.get_player())
	{
		// reset if there are args
		if (fn.nargs > 0)
		{
			int arg = fn.arg(0).asInt();
			set(FULLYEAR, arg);
			if (fn.nargs > 1)
			{
				arg = fn.arg(1).asInt();
