...
namespace gameswf
{
	...
	void	as_global_transform_ctor(const fn_call& fn)
	{
		if (fn.nargs == 1)
		{
			...
			if (character* movie = cast_to<character>(fn.arg(0).asObject()))
			{
	...
	bool	as_transform::set_member(const tu_string& name, const Value& val)
	{
		...
		switch (member)
		{
			case colorTransform:
			{
				m_color_transform = cast_to<as_color_transform>( val.asObject() );
	...
	bool	as_transform::get_member(const tu_string& name, Value* val)
	{
		as_transform_member	member = get_transform_member( name );
		...
		switch( member )
		{
			case colorTransform:
			{
				val->setObject( m_color_transform.get_ptr() );
				return true;
			}
			case concatenatedColorTransform:
			{
