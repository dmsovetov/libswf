...
namespace gameswf
{
	...
	// Remove a previously-added listener.
	{
		...
		ko->m_listeners.remove(fn.arg(0).asObject());
	}
	...
	// Returns the keycode of the last key pressed.
	{
		...
		fn.result->setInt(ko->get_last_key_pressed());
	}
	...
	// Return true if the specified (first arg keycode) key is pressed.
	{
		...
		int	code = fn.arg(0).asInt();

		as_key*	ko = cast_to<as_key>(fn.this_ptr);
		assert(ko);

		fn.result->setBool(ko->is_key_down(code));
	}
