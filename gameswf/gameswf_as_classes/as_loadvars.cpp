...
namespace gameswf
{
	...
	void	as_global_loadvars_ctor(const fn_call& fn)
	{
		...
		fn.result->set_Object(obj.get_ptr());
	}
	...
	bool	as_loadvars::send_and_load(const char * c_url, Object * target, const tu_string & method)
	{
		tu_string host, uri;
	...
	bool	as_loadvars::set_member(const tu_string& name, const Value& val)
	{
	...
	bool	as_loadvars::get_member(const tu_string& name, Value* val)
	{
	...
	void	as_loadvars::copy_to(Object* target)
	{
