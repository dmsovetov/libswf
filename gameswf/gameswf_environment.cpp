...
namespace gameswf
{
	...
	{
		assert(new_size <= array<Value>::size());
	...
	void	vm_stack::drop(int count)
	{
		...
		for (int i = m_stack_size, n = array<Value>::size(); i < n; i++)
		{
			(*this)[i].setUndefined();
			if (--count == 0)
			{
	...
	void vm_stack::clear_refs(hash<Object*, bool>* visited_objects, Object* this_ptr)
	{
		for (int i = 0; i < array<Value>::size(); i++)
		{
			Object* obj = (*this)[i].asObject();
			if (obj)
			{
				if (obj == this_ptr)
				{
...
#if 0
	Object* vm_stack::find_property(const char* name)
	{
	...
	bool vm_stack::get_property(const char* name, Value* val)
	{
		for (int i = size() - 1; i >= 0; i--)
		{
	...
	as_environment::as_environment(player* player) :
		m_player(player)
	{
	//	if (player->get_root() && player->get_root()->is_avm2())    ///!!!Plarium
	//	{
...
/*
	Value	as_environment::get_variable(const tu_string& varname, const array<with_stack_entry>& with_stack) const
	// Return the value of the given var, if it's defined.
	{
		// Path lookup rigamarole.
		Object*	target = get_target();
		tu_string	path;
		tu_string	var;
	...
	Value	as_environment::get_variable_raw(
	...
	// varname must be a plain variable name; no path parsing.
	{
		...
		{
			Object*	obj = with_stack[i].m_object.get_ptr();
		...
		switch (varname_id)
		{
			...
			case M_GLOBAL:
                assert(false);
			//	val.setObject(get_player()->get_global());
				return val;

			case MTHIS:
				val.setObject(get_target());
				return val;

			case M_ROOT:
			case M_LEVEL0:
				val.setObject(get_root()->get_root_movie());
				return val;
		}
...
*/
	...
	void as_environment::set_target(Value& target, character* original_target)
	{
		if (target.isString())
		{
			tu_string path = target.asString();
		...
		if (target.isObject())
		{
			IF_VERBOSE_ACTION(log_msg("-------------- ActionSetTarget2: %s", target.asCString()));
...
/*
	...
	{
		IF_VERBOSE_ACTION(log_msg("-------------- %s = %s\n", varname.c_str(), val.asCString()));//xxxxxxxxxx
	...
	void	as_environment::set_local(const tu_string& varname, const Value& val)
	...
	void	as_environment::add_local(const tu_string& varname, const Value& val)
...
*/
	Value* as_environment::get_register(int reg)
	{
		Value* val = local_register_ptr(reg);
		IF_VERBOSE_ACTION(log_msg("-------------- get_register(%d): %s at %p\n", 
			reg, val->asCString(), val->asObject()));
		return val;
	}

	void as_environment::set_register(int reg, const Value& val)
	{
	...
	Value*	as_environment::local_register_ptr(int reg)
	...
	Object*	as_environment::find_target(const Value& target) const
	{
		if (m_target != NULL)
		{
	...
	bool	as_environment::set_member(const tu_string& name, const Value& val)
	{
		if (m_target != NULL)
		{
	...
	bool	as_environment::get_member(const tu_string& name, Value* val)
	{
		if (m_target != NULL)
		{
	...
	void as_environment::clear_refs(hash<Object*, bool>* visited_objects, Object* this_ptr)
	{
		// target
		...
		{
			Object* obj = m_local_frames[i].m_value.asObject();
			if (obj)
			{
				if (obj == this_ptr)
				{
					m_local_frames[i].m_value.setUndefined();
				}
				else
				{
					obj->clear_refs(visited_objects, this_ptr);
	...
	//	m_scope.clear_refs(visited_objects, this_ptr);
		...
		{
			Object* obj = m_global_register[i].asObject();
			if (obj)
			{
				if (obj == this_ptr)
				{
					m_global_register[i].setUndefined();
				}
				else
				{
					obj->clear_refs(visited_objects, this_ptr);
		...
		{
			Object* obj = m_local_register[i].asObject();
			if (obj)
			{
				if (obj == this_ptr)
				{
					m_local_register[i].setUndefined();
				}
				else
				{
					obj->clear_refs(visited_objects, this_ptr);
