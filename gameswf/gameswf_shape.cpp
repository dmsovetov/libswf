...
namespace gameswf
{
	...
	mesh::mesh() : m_slice_index(-1)	///!!!Plarium
	{
	}
...
//		memcpy(&m_triangle_strip[0], &pts[0], count * sizeof(point));
	...
	{
        render::set_slice_index(m_slice_index);	///!!!Plarium
...
//		memcpy(&m_coords[0], coords, coord_count * sizeof(coords[0]));
	...
	void	mesh_set::display(
		...
		const array<line_style>& line_styles, render_handler::bitmap_blend_mode bm, const scaling_grid* scalingGrid) const ///!!!Plarium
	// Throw our meshes at the renderer.
	{
        ...
        render::set_scaling_grid(scalingGrid); ///!!!Plarium
	...
	///!!!Plarium
    slice9_mesh_set::slice9_mesh_set(const tesselate::tesselating_shape* sh, float error_tolerance) : mesh_set(sh, error_tolerance)
    {

    }

    void slice9_mesh_set::sort_slices(scaling_grid* scalingGrid)
    {
        assert(m_layers.size() == 1);

        int index[9] = {
            scaling_grid::LEFT_TOP,     scaling_grid::CENTER_TOP,    scaling_grid::RIGHT_TOP,
            scaling_grid::LEFT_CENTER,  scaling_grid::CENTER,        scaling_grid::RIGHT_CENTER,
            scaling_grid::LEFT_BOTTOM,  scaling_grid::CENTER_BOTTOM, scaling_grid::RIGHT_BOTTOM,
        };

        const rect& splitter = scalingGrid->m_splitter;

        for( int i = 0, nlayers = m_layers.size(); i < nlayers; i++ ) {
            layer& layer = m_layers[i];

            for( int j = 0, nmeshes = layer.m_meshes.size(); j < nmeshes; j++ ) {
                if( !layer.m_meshes[j] ) {
                    continue;
                }

                // ** Calculate mesh center
                const array<coord_component>& points = layer.m_meshes[j]->get_coords();
                int   count = points.size() / 2;
                rect  bounds;

                bounds.m_x_min = bounds.m_y_min =  FLT_MAX;
                bounds.m_x_max = bounds.m_y_max = -FLT_MAX;

                for( int k = 0; k < count; k++ ) {
                    point p( points[k * 2 + 0], points[k * 2 + 1] );

                    if( p.m_x <= bounds.m_x_min )       bounds.m_x_min = p.m_x;
                    else if( p.m_x >= bounds.m_x_max )  bounds.m_x_max = p.m_x;
                    if( p.m_y <= bounds.m_y_min )       bounds.m_y_min = p.m_y;
                    else if( p.m_y >= bounds.m_y_max )  bounds.m_y_max = p.m_y;
                }

                point center( (bounds.m_x_min + bounds.m_x_max) * 0.5f, (bounds.m_y_min + bounds.m_y_max) * 0.5f );
                int horizontal = 1, vertical = 1;

                // ** Sort this mesh
                if( center.m_x <= splitter.m_x_min ) horizontal = 0;
                else if( center.m_x >= splitter.m_x_max ) horizontal = 2;

                if( center.m_y <= splitter.m_y_min ) vertical = 0;
                else if( center.m_y >= splitter.m_y_max ) vertical = 2;

                int idx = index[vertical * 3 + horizontal];

                scalingGrid->m_bounds[idx] = bounds;
                layer.m_meshes[j]->set_slice_index( idx );
            }
        }

        scalingGrid->calculate();
    }
...
#define SHAPE_LOG 0
	...
	// Draw the shape using our own inherent styles.
	{
		...
		case 0:
		case 1:
			display(mat, cx, pixel_scale, m_fill_styles, m_line_styles, render_handler::BLEND_NORMAL, inst);	///!!!Plarium
			break;
		case 2:
			display(mat, cx, pixel_scale, m_fill_styles, m_line_styles, render_handler::BLEND_LAYER, inst);
			break;
		case 3:
			display(mat, cx, pixel_scale, m_fill_styles, m_line_styles, render_handler::BLEND_MULTIPLY, inst);
			break;
		case 4:
			display(mat, cx, pixel_scale, m_fill_styles, m_line_styles, render_handler::BLEND_SCREEN, inst);
			break;
		case 5:
			display(mat, cx, pixel_scale, m_fill_styles, m_line_styles, render_handler::BLEND_LIGHTEN, inst);
			break;
		case 6:
			display(mat, cx, pixel_scale, m_fill_styles, m_line_styles, render_handler::BLEND_DARKEN, inst);
			break;
		case 7:
			display(mat, cx, pixel_scale, m_fill_styles, m_line_styles, render_handler::BLEND_DIFFERENCE, inst);
			break;
		case 8:
			display(mat, cx, pixel_scale, m_fill_styles, m_line_styles, render_handler::BLEND_ADD, inst);
			break;
		case 9:
			display(mat, cx, pixel_scale, m_fill_styles, m_line_styles, render_handler::BLEND_SUBTRACT, inst);
			break;
		case 10:
			display(mat, cx, pixel_scale, m_fill_styles, m_line_styles, render_handler::BLEND_INVERT, inst);
			break;
		case 11:
			display(mat, cx, pixel_scale, m_fill_styles, m_line_styles, render_handler::BLEND_ALPHA, inst);
			break;
		case 12:
			display(mat, cx, pixel_scale, m_fill_styles, m_line_styles, render_handler::BLEND_ERASE, inst);
			break;
		case 13:
			display(mat, cx, pixel_scale, m_fill_styles, m_line_styles, render_handler::BLEND_OVERLAY, inst);
			break;
		case 14:
			display(mat, cx, pixel_scale, m_fill_styles, m_line_styles, render_handler::BLEND_HARDLIGHT, inst);
			break;
		default:
			display(mat, cx, pixel_scale, m_fill_styles, m_line_styles, render_handler::BLEND_NORMAL, inst);
			break;
		}
	}
...
// 		// xxxxxxx
	...
	void	shape_character_def::display( const matrix& mat, const cxform& cx, float pixel_scale, const array<fill_style>& fill_styles, const array<line_style>& line_styles, render_handler::bitmap_blend_mode bm, const character *inst) const
...
// 		}
        ...
        ///!!!Plarium
        scaling_grid* scalingGrid = NULL;
        if(inst)
        {
            scalingGrid = inst->get_parent()->get_character_def()->get_scaling_grid();
        }
		...
		for (int i = 0, n = m_cached_meshes.size(); i < n; i++)
		{
			...
			{
				// Do it.
				candidate->display(mat, cx, fill_styles, line_styles, bm, scalingGrid);	///!!!Plarium
				return;
			}
		}
        ...
        if(scalingGrid == NULL)
        {
            m = new mesh_set(this, object_space_max_error * 0.75f);
        }
        else
        {
            slice9_mesh_set* slice9mesh = new slice9_mesh_set(this, object_space_max_error * 0.75f);
            slice9mesh->sort_slices(scalingGrid);

            m = slice9mesh;
        }

        m_cached_meshes.push_back(m);
		m->display(mat, cx, fill_styles, line_styles, bm, scalingGrid);
