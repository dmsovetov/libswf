...
{
	template< typename T >
	void stack_push_back_value( array<Value> & stack, T value )
	{
		stack.push_back( value );
	}

	template< typename T >
	void stack_push_back_ref( array<Value> & stack, T & value )
	{
		stack.push_back( value );
	}

	typedef void ( *stack_int_function )( array<Value> & stack, int value );
	typedef void ( *stack_charp_function )( array<Value> & stack, char * value );
	typedef void (array<Value>::*push_value_function )( Value & );
	typedef Value & (array<Value>::*index_function)(int);
	...
	struct callpropvoid
	{
		static void call( const Object *_this, const char* name, int arg_count, array<Value> & stack )
		{
			as_environment env(_this->get_player());
			for (int i = 0; i < arg_count; i++)
			{
				Value& val = stack[stack.size() - 1 - i];
				env.push(val);
			}
			stack.resize(stack.size() - arg_count);

			Object* obj = stack.back().to_object();
			stack.resize(stack.size() - 1);

			Value func;
			if (obj && obj->get_member(name, &func))
			{
				call_method(func, &env, obj,	arg_count, env.get_top_index(), name);
			}
...
#define var_scope jit_getarg( 2 )
		...
		do
		{
			...
			switch (opcode)
			{
				...
				case 0x30:	// pushscope
				{
					...
					typedef Value& ( array<Value>::*back_function )();
					jit_load( m_compiled_code, jit_this_pointer, jit_getarg( 1 ) );
					jit_this_call( m_compiled_code, (back_function)&array<Value>::back );

					jit_load( m_compiled_code, jit_this_pointer, var_scope );
					jit_push( m_compiled_code, jit_result );
					jit_call( m_compiled_code, (push_value_function)&array<Value>::push_back );
				...
				case 0x47:	// returnvoid
				{
					...
					jit_this_call( m_compiled_code, &Value::set_undefined );
				...
				case 0x49:	// constructsuper
				{
					...
					struct constructsuper
					{
						...
						static void call( array<Value> & stack, int arg_count )
						{
							Object* obj = stack.back().to_object();
							stack.resize(stack.size() - 1);
							for (int i = 0; i < arg_count; i++)
				...
				// Stack: …, obj, [ns], [name], arg1,...,argn => …
				{
					...
					for (int i = 0; i < arg_count; i++)
					{
					 ...
					 jit_push( m_compiled_code, jit_esi );
					 jit_this_call( m_compiled_code, & as_environment::push<Value&> );
					 
					 //val--;
					 jit_subi( m_compiled_code, jit_esi, sizeof( Value ) );
					}
					...
					jit_this_call( m_compiled_code, &Value::to_object );
					jit_mov( m_compiled_code, jit_esi, jit_result );
    ...
    /*
				...
				case 0x5D:	// findpropstrict
				{
					...
					{
						static Object * call( char* name, array<Value> & scope )
						{
					...
					jit_push( m_compiled_code, jit_result );
					jit_push( m_compiled_code, jit_esi );
					jit_call( m_compiled_code, &stack_push_back_value<Object*> );
					jit_popargs( m_compiled_code, 2 );
				...
				case 0x5E:	// findproperty, Search the scope stack for a property
				{
					...
					struct findproperty
					{
						static void call( const char* name, array<Value> & scope, array<Value> & stack )
						{
							Object* obj = NULL;
							for (int i = scope.size() - 1; i >= 0; i--)
							{
				...
				case 0x68:	// initproperty, Initialize a property.
				{
					...
					struct initproperty
					{
						static void call( const char * name, array<Value> & stack )
						{
							Value& val = stack[stack.size() - 1];
							Object* obj = stack[stack.size() - 2].to_object();
							if (obj)
							{
				...
				{
					//Value& val = lregister[opcode & 0x03];
					jit_load( m_compiled_code, jit_this_pointer, jit_getarg( 0 ) );
					jit_pushi( m_compiled_code, opcode & 0x03 );
					jit_this_call( m_compiled_code, (index_function)&array<Value>::operator[] );
					jit_popargs( m_compiled_code, 1 );

					//stack.push_back(val);
					jit_push( m_compiled_code, jit_result );
					jit_load( m_compiled_code, jit_this_pointer, jit_getarg( 1 ) );
					jit_this_call( m_compiled_code, (push_value_function) &array<Value>::push_back );
					jit_popargs( m_compiled_code, 1 );
...
#ifdef __GAMESWF_ENABLE_JIT__
		jit_load( m_compiled_code, jit_this_pointer, jit_getarg( 1 ) );
		jit_this_call( m_compiled_code, &array<Value>::size );
		jit_subi( m_compiled_code, jit_eax, count );
		jit_pusharg( m_compiled_code, jit_eax );
		jit_load( m_compiled_code, jit_this_pointer, jit_getarg( 1 ) );
		jit_this_call( m_compiled_code,  &array<Value>::resize );
		jit_popargs( m_compiled_code, 1 );
#endif
	}
}*/
