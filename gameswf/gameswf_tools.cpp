...
namespace gameswf { namespace tools
{
	...
	void	write_placeholder_bitmap(tu_file* out, Uint16 character_id, int width, int height)     ///!!!Plarium
	...
	// with the given character_id.
	{
		...
		out->write_le16(width);     // width, min pitch = 4 bytes/row   ///!!!Plarium
		out->write_le16(height);	// height                           ///!!!Plarium
...
//	IF_VERBOSE_PARSE(m_frame_size.print());
	...
	while ((Uint32) str.get_position() < file_end_pos)
	{
			     ...
			     || tag_type == 36))
		{
            ...
            case 20:
            case 36:    str.read_u8();	// 3 == 8 bit, 4 == 16 bit, 5 == 32 bit
                        width  = str.read_u16();
                        height = str.read_u16();
                        break;
            }
			...
			write_placeholder_bitmap(swf_out, cid, width, height);
		}
		else
		{
