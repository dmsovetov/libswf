...
namespace gameswf
{
	...
	void	SDL_sound_handler::play_sound(Object* listener_obj, int sound_handle, int loops)
	...
	void	SDL_sound_handler::attach_aux_streamer(aux_streamer_ptr ptr, Object* netstream)
	{
		assert(netstream);
		assert(ptr);
	...
	void SDL_sound_handler::detach_aux_streamer(Object* netstream)
	{
		m_mutex.lock();
	...
	static void	sdl_audio_callback (void *udata, Uint8 *stream, int len)
	{
		...
		if (handler->m_aux_streamer.size() > 0)
		{
			...
			for (hash< Object*, gameswf::sound_handler::aux_streamer_ptr>::const_iterator it = handler->m_aux_streamer.begin();
