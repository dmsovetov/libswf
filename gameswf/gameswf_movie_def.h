...
//				printf("signal for frame %d\n", m_loading_frame);
		...
		virtual void	add_abc(tu_string& name, AbcInfo* abc) = 0;
	...
	struct movie_def_impl : public movie_definition_sub
	{
		...
		string_hash<gc_ptr<character_def> > m_exports;
		...
		virtual void	add_abc(tu_string& name, AbcInfo* abc);
		const AbcInfo * get_abc() const{ return m_abc.get_ptr(); }
		...
		// flash9
		virtual Function* instanciateClass(character* ch) const;
	};
