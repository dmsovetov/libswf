...
namespace gameswf
{
	...
	// Constructor for ActionScript class XMLSocket
	{
		...
		fn.result->setObject(ch);
	}

    ///!!!Plarium
    void    as_global_get_text_format(const fn_call& fn)
    {
        ...
        Object* format = fn.result->asObject();

		format->set_member("leftMargin", Value(TWIPS_TO_PIXELS(et->m_left_margin)));
		format->set_member("indent", Value(TWIPS_TO_PIXELS(et->m_indent)));
		format->set_member("rightMargin", Value(TWIPS_TO_PIXELS(et->m_right_margin)));
		format->set_member("leading", Value(TWIPS_TO_PIXELS(et->m_leading)));
	//	format->set_member("letterSpacing", Value(TWIPS_TO_PIXELS(et->m_letter_spacing)));
	//	format->set_member("color", Value(et->m_color.get()));
		format->set_member("size", Value(TWIPS_TO_PIXELS(et->m_text_height)));

		if(et->m_alignment == edit_text_character_def::ALIGN_LEFT)
			format->set_member("align", "left");
		else if(et->m_alignment == edit_text_character_def::ALIGN_CENTER)
			format->set_member("align", "center");
		else if(et->m_alignment == edit_text_character_def::ALIGN_RIGHT)
			format->set_member("align", "right");
		else if(et->m_alignment == edit_text_character_def::ALIGN_JUSTIFY)
			format->set_member("align", "justify");

		format->set_member("font", Value(et->m_font->get_name()));
		format->set_member("bold", Value(et->m_font->is_bold()));
		format->set_member("italic", Value(et->m_font->is_italic()));

	//	if(et->m_image_wrapping_mode == EditTextCharacter::IMAGE_WRAPPING_WIDTH_ONLY)
	//		format->set_member("imageWrapping", "widthOnly");
	//	else
	//		format->set_member("imageWrapping", "normal");
    }
    void    as_global_set_text_format(const fn_call& fn)
    {
		edit_text_character* et = cast_to<edit_text_character>(fn.this_ptr);
		assert(et);
		if (fn.nargs == 1)
		{
			as_textformat* tf = cast_to<as_textformat>(fn.arg(0).asObject());
			et->reset_format(tf);
		}
    }
    ///!!!Plarium
...
//				* pixel_scale;
			...
			for (int j = 0; j < rec.m_glyphs.size(); j++)
			{
				...
				if (g.m_bitmap_info != NULL)
				{
                    ...
                    rect bounds, uv_bounds;

                    float w = g.m_bounds.m_x_max;
                    float h = g.m_bounds.m_y_max;// - g.m_bounds.m_y_min;

                    float a = g.m_bounds.m_y_min;//g.m_fontsize/2.0f; // anchor point
                    float b = g.m_bounds.m_x_min;

                    bounds.m_x_min = PIXELS_TO_TWIPS(  b  ) * s_EM_scale;
                    bounds.m_y_min = PIXELS_TO_TWIPS( -a  ) * s_EM_scale;
                    bounds.m_x_max = PIXELS_TO_TWIPS( w+b ) * s_EM_scale;
                    bounds.m_y_max = PIXELS_TO_TWIPS( h-a ) * s_EM_scale;
...
//					mat.set_scale_rotation(yscale, yscale, radians);
	...
	edit_text_character::edit_text_character(player* player, character* parent,
        ...
        m_rect(def->m_rect),    ///!!!Plarium
        m_paragraph(0),         ///!!!Plarium
		m_cursor(0), 
		m_xcursor(0.0f), 
		m_ycursor(0.0f),
		m_x(0.0f), 
	...
	void edit_text_character::reset_format(as_textformat* tf)
	{
		...
		{
			m_left_margin = PIXELS_TO_TWIPS(val.asFloat());
		}

		if (tf->get_member("indent", &val))
		{
			m_indent = PIXELS_TO_TWIPS(val.asFloat());
		}

		if (tf->get_member("rightMargin", &val))
		{
			m_right_margin = PIXELS_TO_TWIPS(val.asFloat());
		}

		if (tf->get_member("leading", &val))
		{
			m_leading = PIXELS_TO_TWIPS(val.asFloat());
		}
			...
			{ 
				...
				{ 
					...
					if (get_member("onSetFocus", &function))
					{
						...
						env.push(Value());	// oldfocus, TODO
						gameswf::call_method(function, &env, this, 1, env.get_top_index(), "onSetFocus");
					}
		...
		case event_id::KILLFOCUS: 
			{ 
				...
				{ 
					...
					{
						as_environment env(get_player());
						env.push(Value());	// newfocus, TODO
						gameswf::call_method(function, &env, this, 1, env.get_top_index(), "onKillFocus");
					}
	...
	bool edit_text_character::get_topmost_mouse_entity( character * &te, float x, float y, bool shouldHandleMouse ) ///!!!Plarium - shouldHandleMouse
	{ 
		...
		if (m_rect.point_test(p.m_x, p.m_y))
		{
			te = this;
			return true; 
		} 
	...
	// Set our text to the given string.
	{
		...
		{
			Object* ch = get_parent();	// target, default is parent
			tu_string path;
	...
	const char*	edit_text_character::to_string()
	{
		...
		if (get_var_name().size() > 0)
		{
			...
			{
				Value val;
				if (ch->get_member(var, &val) && val.asObject() != this)
				{
					if (val.asString() != m_text)
					{
						set_text(val.asString().c_str());
					}
				}
			}
		}
	...
	bool	edit_text_character::set_member(const tu_string& name, const Value& val)
	// We have a "text" member.
	{
		...
		switch (std_member)
		{
			...
			case M_PASSWORD:
				{
					m_password = val.asBool();
				}break;

			case M_TEXT:
			{
				set_text_value(val.asString());
				break;
			}

            case M_WIDTH:
            {
                m_rect.m_x_max = m_rect.m_x_min + (float)(PIXELS_TO_TWIPS(val.asNumber()));
                format_text();
                return true;
            }
            ...
            case M_HEIGHT:
            {
                m_rect.m_y_max = m_rect.m_y_min + (float)(PIXELS_TO_TWIPS(val.asNumber()));
                format_text();
                return true;
            }
			...
			case M_BORDER:
				m_def->m_border = val.asBool();
				format_text();
				break;

			case M_MULTILINE:
				m_def->m_multiline = val.asBool();
				format_text();
				break;

			case M_WORDWRAP:
				m_def->m_word_wrap = val.asBool();
				format_text();
				break;
			...
			case M_TYPE:
				...
				if (val.asString() == "input")
				{
					m_def->m_readonly = false;
				}
				else
				if (val.asString() == "dynamic")
				{
			...
			case M_BACKGROUNDCOLOR:
				m_background_color = rgba(val.asNumber());
				format_text();
				break;
		}
	...
	bool	edit_text_character::get_member(const tu_string& name, Value* val)
	{
        // first try built-ins sprite methods
		if (get_builtin(BUILTIN_TEXT_METHOD, name, val))
		{
			return true;
		}
		...
		switch (std_member)
		{
			...
			case M_PASSWORD:
				{
					val->setBool( m_password );
				}break;

			case M_TEXT:
				val->setString(m_text);
				return true;

			case M_TEXTCOLOR:
				// Return color in 0xRRGGBB format
				val->setInt((m_color.m_r << 16) + (m_color.m_g << 8) + m_color.m_b);
				return true;
			...
			case M_TEXTWIDTH:
				...
				val->setNumber(TWIPS_TO_PIXELS(m_text_bounding_box.width()));
				return true;

            case M_TEXTHEIGHT:  ///!!!Plarium
                val->setNumber(TWIPS_TO_PIXELS(m_text_bounding_box.height()));
                return true;

            case M_WIDTH:
                val->setNumber(TWIPS_TO_PIXELS(m_rect.m_x_max - m_rect.m_x_min));
                return true;

            case M_HEIGHT:
                val->setNumber(TWIPS_TO_PIXELS(m_rect.m_y_max - m_rect.m_y_min));
                return true;

			case M_BORDER:
				val->setBool(m_def->m_border);
				return true;

			case M_MULTILINE:
				val->setBool(m_def->m_multiline);
				return true;
		
			case M_WORDWRAP:
				val->setBool(m_def->m_word_wrap);
				return true;

			case M_TYPE:
				val->setString(m_def->m_readonly ? "dynamic" : "input");
				return true;

			case M_BACKGROUNDCOLOR:
				val->setInt(m_background_color.m_r << 16 | m_background_color.m_g << 8 |
...
#define WIDTH_FUDGE 80.0f
	...
	//	rec.m_style.m_leading += m_font->get_leading() * rec.m_style.m_scale; ///!!!Plarium

		rec.m_style.m_font = m_font.get_ptr();
		rec.m_style.m_color = m_color;

		rec.m_style.m_x_offset = fmax(0, m_left_margin + m_indent);
	//	rec.m_style.m_y_offset = m_text_height + (m_font->get_leading() - m_font->get_descent()) * rec.m_style.m_scale;///!!!Plarium

		rec.m_style.m_x_offset += m_rect.m_x_min;
		rec.m_style.m_y_offset += m_rect.m_y_min;

		rec.m_style.m_text_height = m_text_height;

        ///!!!Plarium
        rec.m_style.m_y_offset = m_font->get_ascent() * rec.m_style.m_scale + PIXELS_TO_TWIPS(2);
        rec.m_style.m_line_gap = (m_font->get_ascent() + m_font->get_descent()) * rec.m_style.m_scale + rec.m_style.m_leading;
...
//		printf("html paragraph\n");
		...
		while (p <= pend)
		{
			...
			if (strncmp(p, "align=", 6) == 0) 
			{
                ...
                if (strncmp(p, "justify", 7) == 0)  ///!!!Plarium
                {
                    m_alignment = edit_text_character_def::ALIGN_JUSTIFY;
                    p += 8;
                }
	...
	const char*	edit_text_character::html_text(const char* p, text_glyph_record& rec)
	{
		...
		while (true)
		{
			...
			if (strncmp(p, "</i>", 4) == 0)
			{
				if (text.size() > 0)
				{
					format_plain_text(text + (m_paragraph ? "\r" : ""), rec);
					text = "";
				}
				p += 4;
				rec.m_style.m_font->set_italic(false);
	...
	void	edit_text_character::format_plain_text(const tu_string& text, text_glyph_record&	rec)
	{
		...
		while (Uint32 code = utf8::decode_next_unicode_character(&text_ptr))
		{
			...
			if (code == 13 || code == 10)
			{
                ...
                m_y += rec.m_style.m_line_gap;  ///!!!Plarium
			...
			if (m_x >= m_rect.width() - m_right_margin - WIDTH_FUDGE && m_def->m_multiline == true)
			{
